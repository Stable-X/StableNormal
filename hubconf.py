...
def StableNormal(local_cache_dir: Optional[str] = None, device="cuda:0", 
    ...
    x_start_pipeline = YOSONormalsPipeline.from_pretrained(
        yoso_weight_path, trust_remote_code=True, 
        variant="fp16", torch_dtype=torch.float16).to(device)
    
    pipe = StableNormalPipeline.from_pretrained(diffusion_weight_path, trust_remote_code=True,
...
def StableNormal_turbo(local_cache_dir: Optional[str] = None, device="cuda:0", yoso_version='yoso-normal-v1-5') -> Predictor:
